# CodeWiki MCP v1.3.0 — Live Scenario Tests
...
> Prefix prompts with `@codewiki` so the master orchestrator agent handles
...
>
> **Prerequisites**:
> 1. `codewiki-mcp` MCP server running (`C:\Users\alber\anaconda3\Scripts\codewiki-mcp.exe`)
...
|------|--------------------|
| 1 | Master spawns **CodeWiki Researcher** (general exploration) |
| 2 | Researcher calls `codewiki_list_topics(repo_url="vue")` |
| 3 | Tool detects `"vue"` is a bare keyword (`is_bare_keyword()` → True) |
| 4 | Resolver scrapes CodeWiki search for "vue" → multiple results |
...
| 10 | Normal CodeWiki documentation follows |
...
- [x] Actual CodeWiki content follows (not hallucinated)
...
| Step | What should happen |
|------|--------------------|
| 1 | Master spawns **CodeWiki Researcher** |
| 2 | Researcher calls `codewiki_list_topics(repo_url="openclaw")` |
...
- [ ] If NOT_INDEXED: subagent detects it and calls `codewiki_request_indexing`
...
**Trigger**: A keyword so specific that CodeWiki returns exactly one repo.

### Prompt

```
@codewiki Explain the architecture of anthropic-sdk-python
```
...
| Step | What should happen |
|------|--------------------|
| 1 | Master spawns **CodeWiki Architecture Explorer** |
| 2 | Explorer calls a CodeWiki tool with `"anthropic-sdk-python"` |
...
- [ ] Explorer uses `codewiki_read_structure` to map the documentation tree

---

## Scenario 4: GitHub API Fallback — Typo in Keyword

**Feature**: When CodeWiki search returns 0 results, falls back to GitHub REST
API which supports fuzzy matching for typos and misspellings.

### Prompt

```
@codewiki What is flaask?
```
...
|------|--------------------|
| 1 | Master spawns **CodeWiki Researcher** |
| 2 | Researcher calls `codewiki_list_topics(repo_url="flaask")` |
| 3 | Resolver scrapes CodeWiki search for "flaask" → **0 results** |
...
- [ ] CodeWiki search returns 0 results (keyword is misspelled)
...
- [ ] Normal CodeWiki content follows for the resolved repo
- [x] No error / crash from the empty CodeWiki result

### Alternate Typo Prompts

```
@codewiki Tell me about fecbook/react
```

```
@codewiki What does veu do?
```

```
@codewiki Explain ract architecture
```
...
| Step | What should happen |
|------|--------------------|
| 1 | Master spawns **CodeWiki Comparison** ("compare", "vs") |
| 2 | Comparison agent calls CodeWiki tools for "vue" and "react" |
...
**Feature**: `codewiki_request_indexing` asks the user to confirm before
...
|------|--------------------|
| 1 | Master spawns **CodeWiki Researcher** |
| 2 | Researcher calls `codewiki_list_topics("Snowflake-Labs/agent-world-model")` |
| 3 | Returns `NOT_INDEXED` error |
| 4 | Researcher decides to call `codewiki_request_indexing` |
...
- [x] After "Yes" → Playwright navigates to CodeWiki and submits the request
- [x] Response includes "Indexing request submitted successfully"
- [x] Next-step guidance is included (check back, CodeWiki URL, timeline)
...
```
@codewiki What topics does fastapi/fastapi have?
```
...
- [ ] Normal CodeWiki topics returned
...
```
@codewiki Read the architecture section of https://github.com/microsoft/vscode
```
...
- [ ] Content returned from CodeWiki
- [ ] Behaves identically to v1.1.0
...
```
@codewiki What topics does anthropics/anthropic-sdk-python have?
```
...
```
@codewiki Combine the routing system from flask and the async handling from fastapi into a new API framework design.
```
...
| 2 | Master spawns **CodeWiki Synthesizer** |
| 3 | Synthesizer calls CodeWiki tools with `"flask"` and `"fastapi"` |
...
- [ ] All content grounded in CodeWiki data
...
```
@codewiki What is vue?          (1st — resolves + fetches)
@codewiki What is vue?          (2nd — cache hit on resolution, fetches)
@codewiki What is vue?          (3rd — cache hit on resolution, fetches)
@codewiki What is vue?          (4th — should hit rate limit)
```
...
Subsequent calls with the same keyword skip the CodeWiki/GitHub search.

### Test Procedure

```
@codewiki What is vue?                    (1st — full resolution + elicitation)
@codewiki Explain the architecture of vue  (2nd — cache HIT on resolution)
```
...
Expected: CodeWiki search fails → GitHub API fallback catches typo →
resolves "veu" → "vue" → search proceeds.

### Tool: `codewiki_request_indexing`
...
| GitHub API fallback not firing | Keyword found on CodeWiki (fallback only when 0 CodeWiki results) | Use a truly misspelled keyword like "flaask" |
...
|---|---|---|
| 1 | `@codewiki What is vue?` | **Partial pass (elicitation checks unverified)** |
| 2 | `@codewiki What topics does openclaw have?` (mapped from `What is openclaw? and how to use it?`) | **Pass (indexed branch)** |
| 3 | `@codewiki Explain the architecture of anthropic-sdk-python` | **Mostly pass (structure-call check unverified)** |
| 4 | `@codewiki What is flaask?` | **Partial (fallback target mismatch; some checks unverified)** |
| 6 | `@codewiki Check if Snowflake-Labs/agent-world-model is available on CodeWiki` | **Accept path partial pass (elicitation prompt checks unverified)** |
...
1. This run was executed through Copilot chat + `runSubagent` orchestration, not the **VS Code Chat panel `@codewiki` custom-agent runtime**.
2. In this mode, MCP elicitation UI (`ctx.elicit`) is typically not surfaced the same way as in direct `@codewiki` execution.
...
- Re-run Scenarios 1, 4, 5, 6, and 7 in **VS Code Chat panel (Ctrl+Shift+I)** with the literal `@codewiki ...` prompts.
