...
from codewiki_mcp import session_pool
...
async def test_evict_oldest_removes_first(mocker):
    ...
    closer = mocker.patch("codewiki_mcp.session_pool._close_entry")
...
async def test_get_or_create_creates_and_evicts(mocker):
    mocker.patch("codewiki_mcp.session_pool.config.SESSION_POOL_SIZE", 1)
    ...
    evicted = mocker.patch(
        "codewiki_mcp.session_pool._evict_oldest", side_effect=_fake_evict
    )
    created = session_pool._PoolEntry("new", _Closable(), _Closable(), 0)
    mocker.patch("codewiki_mcp.session_pool._create_entry", return_value=created)
...
def test_sync_wrappers_delegate(mocker):
    ...
    mocked = mocker.patch(
        "codewiki_mcp.session_pool.run_in_browser_loop", side_effect=_fake_run
    )
