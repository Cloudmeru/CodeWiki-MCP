...
from codewiki_mcp.server import create_server
...
_HELPERS_RATE = "codewiki_mcp.tools._helpers.check_rate_limit"
_HELPERS_PRE_RESOLVE = "codewiki_mcp.tools._helpers.pre_resolve_keyword"
...
# ---------------------------------------------------------------------------
# codewiki_list_topics — keyword resolution
# ---------------------------------------------------------------------------
class TestTopicsKeywordResolution:
    def test_bare_keyword_resolved_and_note_prepended(self, mocker):
        ...
        mocker.patch(
            "codewiki_mcp.tools.topics.pre_resolve_keyword",
            return_value="vuejs/vue",
        )
        mocker.patch(
            "codewiki_mcp.tools.topics.build_resolution_note",
        ...
        mcp = create_server()
        fn = _get_tool(mcp, "codewiki_list_topics")
...
# ---------------------------------------------------------------------------
# codewiki_read_structure — keyword resolution
# ---------------------------------------------------------------------------
class TestStructureKeywordResolution:
    def test_bare_keyword_resolved(self, mocker):
        mocker.patch(
            "codewiki_mcp.tools.structure.pre_resolve_keyword",
            return_value="vuejs/vue",
        )
        mocker.patch(
            "codewiki_mcp.tools.structure.build_resolution_note",
        ...
        mcp = create_server()
        fn = _get_tool(mcp, "codewiki_read_structure")
        result = fn(repo_url="vue", ctx=_DummyCtx())
        parsed = json.loads(result)
        assert parsed["status"] == "ok"
        assert "Resolved" in parsed["data"]
...
# ---------------------------------------------------------------------------
# codewiki_read_contents — keyword resolution
# ---------------------------------------------------------------------------
class TestContentsKeywordResolution:
    def test_bare_keyword_resolved(self, mocker):
        mocker.patch(
            "codewiki_mcp.tools.contents.pre_resolve_keyword",
            return_value="vuejs/vue",
        )
        mocker.patch(
            "codewiki_mcp.tools.contents.build_resolution_note",
            return_value='> **Resolved:** "vue" → **vuejs/vue**\n',
        )
        page = make_wiki_page(
            repo_name="github.com/vuejs/vue",
        ...
        mcp = create_server()
        fn = _get_tool(mcp, "codewiki_read_contents")
        result = fn(repo_url="vue", ctx=_DummyCtx())
        parsed = json.loads(result)
        assert parsed["status"] == "ok"
        assert "Resolved" in parsed["data"]
...
# ---------------------------------------------------------------------------
# codewiki_search_wiki — keyword resolution
# ---------------------------------------------------------------------------
class TestSearchKeywordResolution:
    def test_bare_keyword_resolved_with_cached_search(self, mocker):
        ...
        mocker.patch(
            "codewiki_mcp.tools.search.pre_resolve_keyword",
            return_value="vuejs/vue",
        )
        mocker.patch(
            "codewiki_mcp.tools.search.build_resolution_note",
            return_value='> **Resolved:** "vue" → **vuejs/vue**\n',
        )
        mocker.patch(
        ...
        mcp = create_server()
        fn = _get_tool(mcp, "codewiki_search_wiki")
    ...
    def test_ctx_optional_for_search(self, mocker):
        """codewiki_search_wiki has ctx as optional (ctx: Context | None)."""
        mocker.patch(
            "codewiki_mcp.tools.search.pre_resolve_keyword",
            side_effect=lambda raw, ctx=None: raw,
        )
