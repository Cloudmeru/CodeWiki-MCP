...
from codewiki_mcp import resolver
...
async def test_parse_search_result_link_absolute_and_relative():
    ...
    assert parsed2.codewiki_url.startswith("https://")
...
def test_fetch_search_results_cache_hit(mocker):
    ...
    mocked = mocker.patch("codewiki_mcp.resolver.run_in_browser_loop")
...
def test_fetch_search_results_handles_errors(mocker):
    mocker.patch(
        "codewiki_mcp.resolver.run_in_browser_loop", side_effect=RuntimeError("boom")
    )
...
def test_resolve_keyword_paths(mocker):
    mocker.patch("codewiki_mcp.resolver._fetch_search_results", return_value=[])
    chosen, rows = resolver.resolve_keyword("vue")
    assert chosen is None
    assert rows == []

    rows = [_result("vuejs", "vue", 99), _result("other", "vue", 1)]
    mocker.patch("codewiki_mcp.resolver._fetch_search_results", return_value=rows)
...
def test_resolve_keyword_interactive_branches(mocker):
    ...
    mocker.patch("codewiki_mcp.resolver._fetch_search_results", return_value=[])
    mocker.patch("codewiki_mcp.resolver._github_search", return_value=[])
    chosen, out = resolver.resolve_keyword_interactive("vscode")
    assert chosen is None and out == []

    mocker.patch("codewiki_mcp.resolver._fetch_search_results", return_value=[rows[0]])
    chosen, out = resolver.resolve_keyword_interactive("vscode")
    assert chosen == "microsoft/vscode"
    assert len(out) == 1

    canonical = [_result("openclaw", "openclaw", 7), _result("x", "y", 1)]
    mocker.patch("codewiki_mcp.resolver._fetch_search_results", return_value=canonical)
...
def test_resolve_keyword_interactive_elicitation_and_fallback(mocker):
    ...
    mocker.patch("codewiki_mcp.resolver._fetch_search_results", return_value=rows)
    mocker.patch(
        "codewiki_mcp.resolver.from_thread.run", return_value="microsoft/vscode"
    )
    chosen, _ = resolver.resolve_keyword_interactive("vscode", ctx=object())
    assert chosen == "microsoft/vscode"

    mocker.patch("codewiki_mcp.resolver.from_thread.run", return_value=None)
    chosen, _ = resolver.resolve_keyword_interactive("vscode", ctx=object())
    assert chosen == "microsoft/vscode"

    mocker.patch(
        "codewiki_mcp.resolver.from_thread.run", side_effect=RuntimeError("no elicit")
    )
    chosen, _ = resolver.resolve_keyword_interactive("vscode", ctx=object())
    assert chosen == "microsoft/vscode"
