"""Tests for codewiki_request_indexing — post-v1.1.0 elicitation & keyword changes.
...
from codewiki_mcp.tools.request_indexing import (
...
from codewiki_mcp.types import ErrorCode
...
class TestRequestIndexingTool:
    def test_validation_error_returns_json(self, mocker):
        ...
        fn = tools["codewiki_request_indexing"].fn
    ...
    def test_keyword_resolution_called(self, mocker):
        ...
        resolve_mock = mocker.patch(
            "codewiki_mcp.tools.request_indexing.pre_resolve_keyword",
            return_value="microsoft/vscode",
        )
        # Mock out everything after validation so we don't hit Playwright
        mocker.patch(
            "codewiki_mcp.tools.request_indexing.from_thread.run",
            return_value=True,  # confirm=True
        )
        mocker.patch(
            "codewiki_mcp.tools.request_indexing._run_request_indexing",
            return_value=__import__("codewiki_mcp.types", fromlist=["ToolResponse"]).ToolResponse.success(
